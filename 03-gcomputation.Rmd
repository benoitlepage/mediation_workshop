...
# 3. Simulate L1 values under the counterfactual scenarios setting A0=0 or A0=1
set.seed(54321)
...
1. Fit a logistic or a linear regression of the final outcome, conditional on the exposure $A$, the mediator $M$ and all the parents of $Y$ preceeding $M$, to estimate $\overline{Q}_{L(2)} = \mathbb{E}(Y \mid L(0),A,L(1),M)$;
                                            
2. Use this estimate to predict an outcome for each subject $\hat{\overline{Q}}_{L(2)}(M=m)_i$, by evaluating the regression fit $\overline{Q}_{L(2)}$ at the chosen value for the mediator $M=m$;

3. Fit a quasibinomial or a linear regression of the predicted values $\hat{\overline{Q}}_{L(2)}(M=m)_i$ conditional on the exposure $A$ and baseline confounders $L(0)$ to estimate $\overline{Q}_{L(1)} = \mathbb{E}\left(\hat{\overline{Q}}_{L(2)}(M=m) \middle| L(0),A\right)$;
