...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1. Estimate gA and gM}
...
In order to illustrate the TMLE procedure, the estimation of a mean counterfactual outcome will be described in detail, following the algorithm implemented in the \texttt{ltmle} package.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Estimate \(\bar{Q}_0\). Data-adpative methods can be used here, the \texttt{ltmle} package relies on the \texttt{SuperLearner} package to fit and predict \(\hat{\bar{Q}}(A=a)\).
\item
  Estimate the treatment mechanism \(g(A=1 \mid L(0))\). Once again, data-adpative methods can be used to improve the estimation.
\end{enumerate}
