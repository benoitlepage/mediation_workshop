...
\item
  Estimate the joint distribution of time-varying confounders \(L(1)\) and of the mediator (\(M_{G_{A=0}}\) and \(M_{G_{A=1}}\)) under the counterfactual scenarios setting \(A = 1\) or \(A=0\);
\item
...
\CommentTok{\# direct and indirect effects on death (binary) and QoL (continuous outcome)}
...
95\% confidence intervals can be calculated by repeating this algorithm in 500 bootstrap samples of the original data set.
