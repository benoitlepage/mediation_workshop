...
\CommentTok{\# 2. Predict an outcome for each subject, setting A=0 and A=1}
...
\CommentTok{\# 3. Plug the predicted outcome in the gformula and use the sample mean }
\CommentTok{\#    to estimate the ATE}
...
The g-formula of the components of a CDE (\(\mathbb{E}(Y_{A=a^\prime,M=m})\)) is more complex than for the average total effect, and the simple substitution approach described previously is less convenient to apply:
...
Several g-computation procedures have been described to estimate CDE:

\begin{itemize}
\tightlist
\item
  parametric g-computation, using Monte Carlo simulations
\item
...
\CommentTok{\# 3. Simulate L1 values under the counterfactual scenarios setting A0=0 or A0=1}
...
\CommentTok{\# 4. Estimate mean outcomes under the counterfactual scenarios setting different }
\CommentTok{\#    levels of exposures for A and M:}
\CommentTok{\#    \{A=0, M=0\} or \{A=1, M=0\} or \{A=0, M=1\} or \{A=1, M=1\}}
...
The following steps describe the implementation of the g-computation estimator by iterative conditional expectation for the component \(\mathbb{E}(Y_{A=a^\prime,M=m})\) used in the definition of CDE \(\Psi^{\text{CDE}_m} = \mathbb{E}(Y_{A=1,M=m}) - \mathbb{E}(Y_{A=0,M=m})\).

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Fit a logistic or a linear regression of the final outcome, conditional on the exposure \(A\), the mediator \(M\) and all the parents of \(Y\) preceeding \(M\), to estimate \(\overline{Q}_{L(2)} = \mathbb{E}(Y \mid L(0),A,L(1),M)\);
\item
  Use this estimate to predict an outcome for each subject \(\hat{\overline{Q}}_{L(2)}(M=m)_i\), by evaluating the regression fit \(\overline{Q}_{L(2)}\) at the chosen value for the mediator \(M=m\);
\item
  Fit a quasibinomial or a linear regression of the predicted values \(\hat{\overline{Q}}_{L(2)}(M=m)_i\) conditional on the exposure \(A\) and baseline confounders \(L(0)\) to estimate \(\overline{Q}_{L(1)} = \mathbb{E}\left(\hat{\overline{Q}}_{L(2)}(M=m) \middle| L(0),A\right)\);
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1) Regress the outcome on L0, A, L1 and M (and the A*M interaction if appropriate)}
\NormalTok{Y.death.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Y\_death }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace }\SpecialCharTok{+}\NormalTok{ L1 }\SpecialCharTok{+}
\NormalTok{                                  M\_smoking }\SpecialCharTok{+}\NormalTok{ A0\_ace}\SpecialCharTok{:}\NormalTok{M\_smoking,}
                        \AttributeTok{family =} \StringTok{"binomial"}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df2\_int)}
\NormalTok{Y.qol.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Y\_qol }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace }\SpecialCharTok{+}\NormalTok{ L1 }\SpecialCharTok{+}
...
\CommentTok{\# 2) Generate predicted values by evaluating the regression setting the mediator}
\CommentTok{\#    value to M=0 or to M=1}
...
\CommentTok{\# 3) Regress the predicted values conditional on the exposure A}
\CommentTok{\#    and baseline confounders L(0)}
\NormalTok{L1.death.Mis0.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Q.L2.death.Mis0 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace,}
                         \AttributeTok{family =} \StringTok{"quasibinomial"}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df2\_int)}
\NormalTok{L1.death.Mis1.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Q.L2.death.Mis1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace,}
                         \AttributeTok{family =} \StringTok{"quasibinomial"}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df2\_int)}

\NormalTok{L1.qol.Mis0.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Q.L2.qol.Mis0 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace,}
                         \AttributeTok{family =} \StringTok{"gaussian"}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df2\_int)}
\NormalTok{L1.qol.Mis1.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Q.L2.qol.Mis1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace,}
                         \AttributeTok{family =} \StringTok{"gaussian"}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df2\_int)}

\CommentTok{\# 4) generate predicted values by evaluating the regression at exposure }
\CommentTok{\#    of interest: \{A=1\} \& \{A=0\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1) Regress the outcome on past}
\NormalTok{Y.qol.model }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Y\_qol }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L0\_male }\SpecialCharTok{+}\NormalTok{ L0\_parent\_low\_educ\_lv }\SpecialCharTok{+}\NormalTok{ A0\_ace }\SpecialCharTok{+}\NormalTok{ L1 }\SpecialCharTok{+} 
\NormalTok{                              M\_smoking }\SpecialCharTok{+}\NormalTok{ A0\_ace}\SpecialCharTok{:}\NormalTok{M\_smoking, }
                   \AttributeTok{family =} \StringTok{"gaussian"}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df2\_int)}

\CommentTok{\# 2) Calculate a residual outcome Y {-} (coef.M * M\_smoking) {-} (coef.A0:M * A0:M)}
...
\CommentTok{\# 3) Regress the residual outcome on the exposure A and baseline confounders L(0)}
...
\CommentTok{\# 4) Use coefficients estimated from the 1st and 2nd regression to estimate CDE:}
...
\CommentTok{\# 2. Predict each individual\textquotesingle{}s probability of being exposed to her own exposure}
...
\CommentTok{\# 3. Apply weights corresponding to the inverse of the predicted probability}
\NormalTok{wt }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{/}\NormalTok{ gA.L}

\CommentTok{\# 4. Use the empirical mean of the weighted outcome}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 3. For example, applying g\^{}*(A) = 1}
\CommentTok{\# 4. Applying the stabilized estimator}
\CommentTok{\# point estimates:}
...
\FunctionTok{write.csv}\NormalTok{(df1, }\AttributeTok{file =} \StringTok{"df1.csv"}\NormalTok{, }\AttributeTok{row.names =} \ConstantTok{FALSE}\NormalTok{)}
...
\FunctionTok{write.csv}\NormalTok{(df1\_int, }\AttributeTok{file =} \StringTok{"df1\_int.csv"}\NormalTok{, }\AttributeTok{row.names =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{write.csv}\NormalTok{(df2, }\AttributeTok{file =} \StringTok{"df2.csv"}\NormalTok{, }\AttributeTok{row.names =} \ConstantTok{FALSE}\NormalTok{)}
...
\FunctionTok{write.csv}\NormalTok{(df2\_int, }\AttributeTok{file =} \StringTok{"df2\_int.csv"}\NormalTok{, }\AttributeTok{row.names =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
