...
\hypertarget{msm-for-the-average-total-effect-ate}{%
\section{MSM for the Average Total Effect (ATE)}\label{msm-for-the-average-total-effect-ate}}
...
For example, in order to fit the MSM \eqref{eq:MSMATEsex} described above, we can use a linear regression of the (observed) outcome \(Y\) on the exposure and sex, weigthed by individual weights \(w_i\):
\begin{equation} 
...
\CommentTok{\# 3. Define individual weights:}
...
\CommentTok{\# Or alternatively, we can use stabilized weights : sw = g(A=a\_i) / g(A=a\_i | L(0))}
...
\CommentTok{\# 4. Estimate coefficients of the MSM using a weighted regression E(Y | A, sex)}
...
\tightlist
\item
  Fit the MSM \(E\left[Y_a \mid L\text{male}(0)\right]\) using the long dataset.
\end{enumerate}
...
\CommentTok{\# Indeed, applying a gaussian family (additive model) with no interaction terms}
\CommentTok{\# implies the assumption of some interaction terms in a multiplicative model (such}
\CommentTok{\# as a glm with a binomial family).}
